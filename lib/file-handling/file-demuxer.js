...
FileDemuxer.prototype._handleSourceBytes = function(chunk) {
  console.log('handleing source bytes')
  console.log(chunk)
  if (!this._currentOutput) {
    console.log('not _currentOutput')
    this._currentOutput = new stream.Readable({ read: utils.noop });
  ...
  if (this._needsNewOutputStream()) {
    console.log('needs new output stream')
...
FileDemuxer.prototype._handleSourceEnded = function() {
  console.log('handleSourceEnded called, finished pushing data to read stream')
  this._closeCurrentOutput();
  this.emit('finish');
};
...
FileDemuxer.prototype._pushBytesToOutput = function(bytes) {
  console.log('pushing bytes to output')
  console.log(bytes)
  if (bytes) {
    this._filePosition += bytes.length;
  }
  console.log('_currentOutput stream')
  console.log(this._currentOutput)
  this._currentOutput.push(bytes);
  // close stream here could cause issues for multiple pushes
...
FileDemuxer.prototype._closeCurrentOutput = function() {
  console.log('_closeCurrentOutput called')
