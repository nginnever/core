...
=======
  if(options.store === undefined) {
    console.log('blacklist store is undefined')
    if(options.blacklistFolder === undefined) {
>>>>>>> Stashed changes
 ...
 * @param {Function} callback
 */
BridgeClient.prototype.storeFileInBucket = function(options, cb) {
  this._opts = options
  var self = this;
  ...
  if (typeof options.stream === 'string') {
    file = fs.createReadStream(options.string);
  }
...
BridgeClient.prototype._startDemuxing = function(options, cb) {
  var self = this;
  const {fileSize, fileName, stream, id} = options;

  //fileName.on('data', ondata);
  // var cs = Concat(gotEncFile)
  ...
  var shardSize = FileDemuxer.getOptimalShardSize(
    {
      fileSize: fileSize,
      shardConcurrency: self._transferConcurrency
    }
  );
  ...
  var uploadState = new UploadState({
    ...
    file: stream,
    onComplete: cb,
    worker: self._shardUploadWorker.bind(self),
    numShards: Math.ceil(fileSize / shardSize),
    concurrency: self._transferConcurrency
  });
...
BridgeClient.prototype._shardUploadWorker = function(task, done) {
  ...
  self._handleShardTmpFileFinish(task.shard, task.state, task.meta, done);
  
  // task.shard.on('data', function(data) {
  //   task.meta.size += data.length;
  //   task.meta.hasher.update(data);

  //   //task.meta.stream.write(data).on('end', () => {
  //   //self._store().createWriteStream({key:'test'}).end('beans', ()=>{
  //   //  console.log('finished writing shard to IDB!!!')
  //   //})

  //   // self._store()._put('test', data, () => {
  //   //   console.log('finished writing shard to IDB!!!')
  //   // })
  //   //console.log('finished writing shard to IDB!!!')
  // }).resume();
  ...
  // task.shard.on('end', () => {
  //   task.meta.hash = task.meta.hasher.digest();
  //   var tester = self._store().createWriteStream({ key: 'cool' }).end('beans', readback);
  //   var strum = new Readable()
  //   strum.push(buffer)
  //   strum.push(null)
  //   strum.pipe(tester)
  //   function readback () {
  ...
  //   }
    ...
    //self._handleShardTmpFileFinish(task.state, task.meta, done);
    //task.meta.stream.end.bind(task.meta.stream)
  //});
 ...
 * @param {String} options.fileName - Key of the file in the blob store
 */
BridgeClient.prototype._handleShardStream = function(options) {
  const {shardStream, index, frame, uploadState, fileName} = options;
  var meta = {
    frame: frame,
    //stream: this._store().createWriteStream({key:'test'}),
    tmpName: fileName,
    size: 0,
    index: index,
    hasher: crypto.createHash('sha256'),
    hash: null,
    excludeFarmers: this._blacklist.toObject(function(){}),
    transferRetries: 0
  };
 ...
 */
BridgeClient.prototype._handleShardTmpFileFinish = function(stream, state, meta, done) {
  var self = this;
  var hash = utils.rmd160(meta.hash);
  var auditGenerator = new AuditStream(3);
  //var ws = meta.stream;
  //var ws = self._store().createReadStream({key:'cool'})
  ...
  self._blacklist.toObject(function(e, blacklist) {
    if (meta.challenges && meta.tree) {
      ...
      stream.pipe(auditGenerator).on('finish', _getContract, blacklist);
    }
  });
};
...
BridgeClient.prototype._shardTransferComplete = function(state, frame, done) {
  ...
  function _shardTransferComplete() {
    ...
    self._request('POST', '/buckets/' + state.bucketId + '/files', {
      frame: frame.id,
      mimetype: mime.lookup(self._opts.fileName),
