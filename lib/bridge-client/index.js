...
BridgeClient.prototype.addShardToFileStagingFrame = function(f, s, opt, cb) {
  console.log('adding file to staging frame')
  ...
  function _addShard() {
    console.log('Querying bridge for contract for retries: ' + retries)
    self._logger.info(
 ...
 * @param {Function} callback
 */
BridgeClient.prototype.storeFileInBucket = function(id, token, file, store, cb) {
  console.log('MADE IT TO CORE')
  var self = this;
  ...
  console.log('got optimal shard size: ' + shardSize)
  ...
  console.log('number of shards: ' + uploadState.numShards)

  function _createFileStagingFrame() {
    console.log('Creating file staging frame')
    ...
    self.createFileStagingFrame(function(err, frame) {
      ...
      demuxer.on('shard', function(shardStream, index) {
        console.log('shard emitted from handleShard, stream below, index: '+ index)
        console.log(shardStream)
        self._handleShardStream(shardStream, index, frame, uploadState, store);
      }).on('error', cb);
    });
  }
...
BridgeClient.prototype._shardUploadWorker = function(task, done) {
  var self = this;
  console.log('upload worker task ')
  console.log(task)
  self._logger.info(
  ...
  console.log('Trying to upload shard %s index %s' +task.meta.tmpName+task.meta.index)
  console.log(task.tmpFile)
  task.state.cleanQueue.push(task.meta.tmpName);

  var storeStream = new chunkStoreStream.write(task.tmpFile, self._shardSize);

  task.shard.pipe(storeStream)
  ...
  console.log('test1')
  task.shard.on('end', () => {
    console.log('finished task shard writing to blob store')
 ...
 */
BridgeClient.prototype._handleShardStream = function(shard, i, frame, state, store) {
  console.log('handling shard stream')
  var self = this;
  self._store = store;
...
BridgeClient.prototype._handleShardTmpFileFinish = function(state, meta, done) {
  ...
  // get read stream from memstore!
  console.log('getting shard from memstore')
  console.log(self._store)
  self._store.get(0, (err, chunk) => {
    self._memChunk = chunk;
    var shardFile = new Readable();
    shardFile.push(chunk)
    shardFile.push(null)

    console.log('got shard from mem store')

    console.log('Hash for this shard is: '+ hash)
    ...
    function _getContract() {
      ...
      if (!meta.challenges && !meta.tree) {
        ...
        console.log('Audit generation for shard done.')
        self._logger.info('Audit generation for shard done.');
      }
      console.log('Waiting on a storage offer from the network...')
      self._logger.info('Waiting on a storage offer from the network...');

      console.log('meta frame id: ' + meta.frame.id)
      ...
      var addShardToFrame = self.addShardToFileStagingFrame(meta.frame.id, {
        ...
        console.log('finished staging frame, starting xfer')
    ...
    if (meta.challenges && meta.tree) {
      ...
      console.log('piping shardfile to audit generator')
...
BridgeClient.prototype._startTransfer = function(pointer, state, meta, done) {
  console.log('begin transfering shard')
  var self = this;
  ...
  console.log('Contract negotiated with: ')
  console.log(pointer.farmer)
...
BridgeClient.prototype._shardTransferComplete = function(state, frame, done) {
  ...
  state.completed++;
  console.log('Shard transfer completed! remaining...' + state.numShards - state.completed)
  this._logger.info(
  ...
  function _shardTransferComplete() {
    console.log('Transfer finished, creating entry.. ')
    console.log('original file name')
    console.log(origFileName)
    origFileName = 'test.png'
    console.log('mimetype')
    console.log(mime.lookup(origFileName))
...
BridgeClient.prototype._transferShard = function(evt, name, pointer, state) {
  console.log('in transfer shard, name (path to file)')
  console.log(name)
  var self = this;
  //var shardFile = fs.createReadStream(name);
  var shardFile = new Readable();
  shardFile.push(self._memChunk)
  shardFile.push(null)
  ...
  shardFile.pipe(uploader).on('finish', function() {
    console.log('finished piping shard file to uploader')
